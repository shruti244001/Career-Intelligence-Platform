...
### 10.6.1 Application Workflow Orchestration

CareerGraph uses an application-level workflow boundary to coordinate
multi-step readiness operations without duplicating domain or service logic.

The current readiness workflow is implemented by `CareerReadinessWorkflow`
under the application layer.

Its responsibility is orchestration only:

- retrieve and validate the interview session
- invoke interview evaluation
- update candidate skill state
- generate target-specific skill gaps
- generate next-best-action recommendations
- return the complete readiness result

The workflow does not implement scoring, proficiency mapping, skill-gap
classification, or recommendation rules itself. Those responsibilities remain
inside their respective domain/application services.
...
```
This establishes the current implemented readiness loop:
```text
Interview
    ↓
Response
...
Recommendation
```
The workflow is deterministic and provider-independent. AI providers may
participate in individual generation or interpretation steps through explicit
infrastructure boundaries, but workflow sequencing, validation, persistence,
state transitions, and deterministic calculations remain outside the LLM.

The current implementation is an in-memory application foundation. Persistent
repositories and production workflow infrastructure will be introduced as
part of the data and Google Cloud implementation phases.
...
The interview API exposes the lifecycle required for deterministic interview
planning, question execution, response capture, and completion.
...
| GET | `/api/v1/interviews/{interview_id}` | Retrieve interview session |
| POST | `/api/v1/interviews/{interview_id}/start` | Start interview session |
| POST | `/api/v1/interviews/{interview_id}/questions/next` | Generate and persist next interview question |
| POST | `/api/v1/interviews/{interview_id}/responses` | Record candidate response |
| POST | `/api/v1/interviews/{interview_id}/complete` | Complete interview session |
...
RECOMMENDATION
```
Question generation is provider-independent at the application boundary.
The API does not expose Gemini-specific implementation details.
...
Interview evaluation is orchestrated by `InterviewEvaluationService`.

The service:

1. validates that the interview is completed
2. retrieves persisted interview questions and responses
3. converts responses into traceable evidence through
   `InterviewEvidenceService`
4. invokes `EvaluationService`
5. produces a `WeightedEvaluation`

The service does not own the underlying scoring rules. Deterministic scoring
and aggregation remain within the evaluation/scoring layers.

### 10.16.1 Career Readiness Orchestration

The application currently provides a deterministic readiness workflow through
...
RecommendationService
```
The workflow produces a `CareerReadinessResult` containing:

- `WeightedEvaluation`
- updated `SkillState` values
- generated `SkillGap` values
- generated `Recommendation` values

This establishes an end-to-end application-level readiness cycle without
requiring an LLM for deterministic state transitions or calculations.

The workflow is intentionally provider-independent and can later be invoked
from synchronous API requests, asynchronous workflow execution, or Google
Cloud event-driven infrastructure without changing the underlying domain
logic.
...
The deterministic application foundation currently implements the core
readiness path from interview execution through evaluation, skill-state
update, skill-gap generation, and recommendation generation.
