...
class DimensionEvaluation(BaseModel):
    """Single rubric dimension evaluation grounded in candidate evidence."""
    ...
    def validate_text_tuples(
        ...
        """Ensure all text entries are non-empty."""
    ...
    def validate_score_and_proficiency(
        self,
    ) -> "DimensionEvaluation":
        """Enforce score, proficiency, and coverage invariants."""

        # No evidence means no score and no proficiency claim.
        ...
        # Partial or sufficient coverage must have a score.
        if self.score is None:
            raise ValueError(
                "dimension evaluation with sufficient or partial "
                "coverage must have a score"
            )

        # A scored evaluation cannot claim insufficient proficiency.
        if (
            self.proficiency
        ...
        CANONICAL_SCORE_SCALE.validate_score(
            self.score
        )
        ...
        if self.proficiency != expected_proficiency:
            raise ValueError(
                f"score {self.score} maps to "
                f"proficiency {expected_proficiency}, "
